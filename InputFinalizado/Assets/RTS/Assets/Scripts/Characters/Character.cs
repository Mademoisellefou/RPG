...
public class Character : MonoBehaviour {
	...
	private string attackCastleTag;
	...
	void Start(){
		...
		attackTag = "Enemy Castle";
		attackCastleTag = attackTag;
	...
	void Update(){
		...
		if(!goingToClickedPos){
			...
			//if currentTarget is out of range...
				...
				if(castle != null && Vector3.Distance(transform.position, castleAttackPosition) <= castleStoppingDistance){
    ...
    if(selected && ((Input.GetMouseButtonDown(1) && !GameObject.Find("Mobile")) || (Input.GetMouseButtonDown(0) && GameObject.Find("Mobile") ))){
